...
declare namespace effectKit {
  ...
  /**
   * A class used to add a specified effect to an image. Before calling any method of Filter, use createEffect to create
   * a Filter instance.
   *
   * @syscap SystemCapability.Multimedia.Image.Core
   * @crossplatform [since 14]
  ...
  interface Filter {
     ...
     * @param { double } radius - Blur radius, in pixels. The blur effect is proportional to the configured value.
     *     A larger value indicates a more obvious effect.
     * @param { TileMode } tileMode - Tile mode of the shader effect. The blur effect of image edges is affected.
     *     Currently,
     ...
     *
     * @param { double } bright - Brightness value, ranging from 0 to 1. When the value is 0, the image brightness
     *     remains unchanged.
     * @returns { Filter } Final image effect.
     * @syscap SystemCapability.Multimedia.Image.Core
     * @crossplatform [since 14]
   ...
   * @returns { Filter } Head node of the filter linked list without any effect. If the operation fails, null is
   *     returned.
   * @syscap SystemCapability.Multimedia.Image.Core
   * @crossplatform [since 14]
   * @form [since 12]
   ...
   *     respectively.
   *     The value of each element must be in the range [0, 1]. The leftmost and topmost positions of the image
   *     correspond to 0,
   *     and the rightmost and bottom positions correspond to 1. In the array, the third element must be greater than
   *     the first element,
