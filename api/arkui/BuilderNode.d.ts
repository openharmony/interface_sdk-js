 ...
 *
 * Compared with **BuilderNode**, **ReactiveBuilderNode** can generate a component tree through the stateless UI
 * method @Builder with multiple parameters.
 *
 * > **NOTE**
 * >
...
export class BuilderNode<Args extends Object[]> {
   ...
   * API, this API can use the builder configuration parameters to determine whether @Builder can be nested with @
   * Builder.
   *
   * > **NOTE**
   *
   ...
   * >
   * > - When an [axis event]{@link AxisEvent} event is injected, it cannot trigger
   * > [rotation gestures]{@link ../@internal/component/ets/gesture}, because the axis event does not include rotation
   * > axis information.
   * >
   ...
   * > coordinate offsets. This is because [TouchType]{@link TouchType} does not change during the event conversion. For
   * > details about the specifications, see [onTouch]{@link CommonMethod#onTouch}.
   * >
   * > - When an [axis event]{@link AxisEvent} event is injected, it cannot trigger
   * > [rotation gestures]{@link ../@internal/component/ets/gesture}, because the axis event does not include rotation
   * > axis information.
   ...
   * @param { CompetitionStrategy } [competitionStrategy] - Whether the gesture for posting the event is in a
   *     competition scenario. By default, the gesture is not in a competition scenario.
...
export class ReactiveBuilderNode<Args extends Object[]> {
   ...
   * instances decorated by the V2 decorator (such as @ObservedV2), the data can be automatically updated without manual
   * calling.
   *
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   ...
   * > the outer layer, as this may cause coordinate offsets. This is because [SourceType]{@link SourceType} does not
   ...
   *
   * @param { boolean } enabled - Whether the current **ReactiveBuilderNode** object inherits the freeze policy from its
   *     parent component's custom components. The value **true** means to inherit the freeze policy from parent
