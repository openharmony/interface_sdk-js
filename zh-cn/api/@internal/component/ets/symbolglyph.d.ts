...
interface SymbolGlyphInterface {
   ...
   * 定义SymbolGlyph组件构造函数。
   *
   * @param { Resource } value - SymbolGlyph组件的资源名，如 $r('sys.symbol.ohos_wifi')。不传入时不显示图标。
...
declare class ScaleSymbolEffect extends SymbolEffect {
   ...
   * @param { EffectDirection } [direction] - 动效方向。具体枚举值及说明请参考EffectDirection枚举说明。
   *     <br>默认值：EffectDirection.DOWN
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform [since 20]
   * @form
...
declare class HierarchicalSymbolEffect extends SymbolEffect {
   ...
   *
   * @param { EffectFillStyle } [fillStyle] - 动效模式。具体枚举值及说明请参考EffectFillStyle枚举说明。
   *     <br>默认值：EffectFillStyle.CUMULATIVE
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform [since 20]
   * @form
...
declare class ReplaceSymbolEffect extends SymbolEffect {
   ...
   *
   * @param { EffectScope } [scope] - 动效范围。
   *     <br>默认值：EffectScope.LAYER
   * @param { ReplaceEffectType } [replaceType] - 替换动效类型。
   *     <br>默认值：ReplaceEffectType.SEQUENTIAL
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   * @form
...
declare class SymbolGlyphAttribute extends CommonMethod<SymbolGlyphAttribute> {
   ...
   * > 从API version 12开始，该接口支持在[attributeModifier]{@link CommonMethod#attributeModifier}中调用。
   *
   * @param { Array<ResourceColor> } value - SymbolGlyph组件字体颜色。
   *     <br> 当value为undefined时，使用图标的默认颜色，默认颜色跟随主题。
   *     <br>不同渲染策略下颜色设置效果不同，详见[SymbolRenderingStrategy]{@link SymbolRenderingStrategy}枚举说明。
  ...
  /**
   * 设置SymbolGlyph组件图标小符号的粗细，支持通过FontWeightConfigs配置是否开启可变字重调节（启用后可设置非100整数倍的精细字重值，如220、660）、是否开启随设备的字体粗细级别自动更新字重（启用后组件字
   * 重随系统字体粗细设置自动调整）。未通过该接口设置时，默认字体粗细为FontWeight.Normal（正常粗细，对应数值400）。
   *
   * @param { number | FontWeight | ResourceStr } value - SymbolGlyph组件图标小符号的粗细。
   *     <br>number类型取值[100, 900]，取值间隔为100，默认为400，取值越大，字体越粗。string类型仅支持number类型取值的字符串形式，例如“400”，以及“bold”、“bolder”、“
   ...
   * > [symbolEffect]{@link SymbolGlyphAttribute#symbolEffect(symbolEffect: SymbolEffect, isActive?: boolean)}接口。两者不可同时使
   * > 用，详见[symbolEffect]{@link SymbolGlyphAttribute#symbolEffect(symbolEffect: SymbolEffect, isActive?: boolean)}接口说明。
   *
   * @param { SymbolEffectStrategy } value - SymbolGlyph组件动效策略。
   * @returns { SymbolGlyphAttribute } The attribute of the SymbolGlyph.
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform [since 20]
   ...
   * > 从API version 12开始，该接口支持在[attributeModifier]{@link CommonMethod#attributeModifier}中调用。
   *
   * @param { SymbolRenderingStrategy } value - SymbolGlyph组件渲染策略。
   * @returns { SymbolGlyphAttribute } The attribute of the SymbolGlyph.
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform [since 20]
   ...
   * > 动效属性，仅支持使用effectStrategy属性或单个symbolEffect属性，不支持多种动效属性混合使用。
   *
   * @param { SymbolEffect } [symbolEffect] - SymbolGlyph组件动效策略。
   * @param { boolean } [isActive] - SymbolGlyph组件动效播放状态。
   *     <br>true表示播放，false表示不播放。
   * @returns { SymbolGlyphAttribute } The attribute of the SymbolGlyph.
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform [since 20]
   ...
   *
   * @param { SymbolEffect } [symbolEffect] - SymbolGlyph组件动效策略。
   * @param { number } [triggerValue] - SymbolGlyph组件动效播放触发器，在数值变更时触发动效。
   *     <br>如果首次不希望触发动效，设置-1。
   * @returns { SymbolGlyphAttribute } The attribute of the SymbolGlyph.
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   ...
   * [fontColor]{@link SymbolGlyphAttribute#fontColor(value: Array<ResourceColor>)}。
   *
   * @param { Array<ShaderStyle | undefined> | ShaderStyle } shader - 径向渐变或线性渐变或纯色。
