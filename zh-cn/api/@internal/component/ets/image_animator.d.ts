...
interface ImageFrameInfo {
  /**
   * 图片路径，图片格式为jpg、jpeg、svg、png、bmp、webp、ico和heif，从API version9开始支持[Resource]{@link Resource}类型的路径，从API version 12开始支持
   * [PixelMap]{@link PixelMap}类型。
   *
   * **string格式说明：**
   *
   * - 支持加载本地图片路径和网络图片地址。使用相对路径引用本地图片时，不支持跨包或跨模块调用。resources目录下的文件不支持通过相对路径访问，需使用[Resource]{@link Resource}类型（如$r或$
   ...
   * [fileUri.getUriFromPath(path)]{@link @ohos.file.fileuri:fileUri.getUriFromPath}方法将路径转换为应用沙箱URI，然后传入显示。同时需要保证目录包路径下的
   * 文件有可读权限。
   * - 支持`Base64`字符串。
   *
...
declare class ImageAnimatorAttribute extends CommonMethod<ImageAnimatorAttribute> {
   ...
   *
   * @param { Array<ImageFrameInfo> } value - 设置图片帧信息集合。每一帧的帧信息(ImageFrameInfo)包含图片路径、图片大小、图片位置和图片播放时长信息，详见
  ...
  /**
   * 设置播放时长。当[images]{@link ImageAnimatorAttribute#images}中任意一帧图片设置了单独的duration后，该属性设置无效。
   *
  ...
  /**
   * 设置当前播放方向下，动画开始前和结束后的状态。动画结束后的状态由fillMode和reverse属性共同决定。例如，fillMode为Forwards表示停止时维持动画最后一个关键帧的状态，若reverse为false则维持正播的
   ...
   * 的可见性判定，控制组件的暂停和播放。
   *
   * @param { boolean } monitorInvisibleArea - true时，组件基于系统的
   *     [onVisibleAreaChange]{@link CommonMethod#onVisibleAreaChange(ratios: Array<number>, event: VisibleAreaChangeCallback)}
   *     可见性判定控制暂停和播放；当组件的运行状态为[AnimationStatus]{@link AnimationStatus}的Running时，若判定不可见则自动暂停，若判定可见则自动恢复播放。false时，组件的暂停和播
   *     放不受onVisibleAreaChange影响。<br/>默认值：false <br/> **说明：** <br/>当该属性由true动态修改为false时，组件将依据当前的
   *     [AnimationStatus]{@link AnimationStatus}状态进行处理。<br/> 例如，若当前状态为Running且因
   *     [onVisibleAreaChange]{@link CommonMethod#onVisibleAreaChange(ratios: Array<number>, event: VisibleAreaChangeCallback)}
  ...
  /**
   * 状态回调，动画取消时触发。当state被设置为[AnimationStatus.Initial]{@link AnimationStatus}时触发；触发后图片显示回到第一帧（正播）或最后一帧（逆播）。与
   * [onFinish]{@link ImageAnimatorAttribute#onFinish}的区别在于：onCancel对应回到Initial初始状态，onFinish对应动画自然结束或停止（Stopped）状态。
   *
  ...
  /**
   * 状态回调，动画播放完成时（iterations设置的轮次全部播完且动画自然结束）或者停止播放时（state被切换为[AnimationStatus.Stopped]{@link AnimationStatus}）触发。当动画处于
   * [AnimationStatus.Initial]{@link AnimationStatus}状态时返回初始状态不会触发该事件，对应触发的是onCancel。
   *
