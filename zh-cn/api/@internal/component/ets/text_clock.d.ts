...
interface TextClockInterface {
   ...
   * 创建文本时钟组件。
   *
   * @param { object } options - 通过文本显示当前系统时间的组件参数。不传入时使用默认配置，各属性默认值详见TextClockOptions。 [since 8 - 17]
   * @param { TextClockOptions } [options] - 通过文本显示当前系统时间的组件参数。不传入时使用默认配置，各属性默认值详见TextClockOptions。 [since 18]
...
declare class TextClockAttribute extends CommonMethod<TextClockAttribute> {
   ...
   *     <br>Wearable设备上默认值：'#c5ffffff'，其他设备默认值：'#e6182431'
   * @returns { TextClockAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @crossplatform [since 10]
   * @form [since 11]
   ...
   * 设置字体大小。
   *
   * @param { Length } value - 字体大小。fontSize为number类型时，使用fp单位。
   *     <br>字体默认大小16fp。不支持设置百分比字符串，传入百分比字符串时使用默认值。
   * @returns { TextClockAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @crossplatform [since 10]
   * @form [since 11]
   ...
   *     <br>默认值：FontStyle.Normal，表示标准的字体样式（非斜体）。
   * @returns { TextClockAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @crossplatform [since 10]
   * @form [since 11]
   ...
   *     值为400。string类型支持：number类型取值的字符串形式（如400），以及枚举值'lighter'（对应300）、'regular'（对应400）、'medium'（对应500）、'bold'（对应700）、'
   *     bolder'（对应900），分别对应FontWeight中相应的枚举值。
   ...
   *     string> [ <integer> | on | off ]。可设置多个特性，中间用','隔开。例如，使用等宽时钟数字的格式为：'"ss01" on'。
   * @returns { TextClockAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   ...
   * 定制TextClock内容区的方法。
   *
   * @param { ContentModifier<TextClockConfiguration> } modifier - 在TextClock组件上，定制内容区的方法。
   ...
   *     hour: "numeric"}时表示不显示前导0。
   *     <br>默认值：undefined，默认状态下，24小时制显示前导0，12小时制不显示前导0。
   * @returns { TextClockAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
