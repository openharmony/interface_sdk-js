...
declare class ShapeAttribute extends CommonMethod<ShapeAttribute> {
   ...
   * 设置形状的视口。
   * 
   * 视口定义了绘制内容的坐标系统和显示区域。视口的起始点坐标(x, y)和宽高(width, height)决定了绘制内容在组件中的显示位置和范围。当视口范围与组件尺寸不同时，绘制内容会自动缩放适配。视口常用于调整绘制内容的显示比例和
   * 位置。
   *
   * @param { object } value - Viewport绘制属性。
   *     <br>默认值：{x: 0, y: 0, width: 0, height: 0}
   *     <br>异常值undefined和null按照默认值处理。 [since 7 - 17]
   * @param { ViewportRect } value - Viewport绘制属性。
   *     <br>默认值：{x: 0, y: 0, width: 0, height: 0}
   *     <br>异常值undefined和null按照默认值处理。 [since 18]
   * @returns { ShapeAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @FaAndStageModel
   * @crossplatform [since 10]
   ...
   * [attributeModifier]{@link CommonMethod#attributeModifier}动态设置属性方法。适用于需要实现图像变形效果的场景，如图片扭曲、波浪效果等视觉效果。
   * 
   ...
   * > drawPixelMapMesh。
   *
   * @param { Array<any> } value - 长度（row + 1）* （column + 1）* 2的数组，记录扭曲后的位图各个顶点位置。坐标系基于Shape组件显示区域，原点(0,0)位于左上角，x轴向右延伸，y
   *     轴向下延伸。
   *     <br>默认单位：vp 
   *     <br>设置异常值undefined、null时按照空数组处理。
   * @param { number } column - mesh矩阵列数，取值范围≥0。
   *     <br>默认值：0
   *     <br>设置异常值undefined、null、NaN和Infinity时，column参数和row参数按默认值0处理，value参数按空数组处理。
   * @param { number } row - mesh矩阵行数，取值范围≥0。
   *     <br>默认值：0
   *     <br>设置异常值undefined、null、NaN和Infinity时，column参数和row参数按默认值0处理，value参数按空数组处理。
   * @returns { ShapeAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
