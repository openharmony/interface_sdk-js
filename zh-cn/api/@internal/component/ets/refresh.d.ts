 ...
 * > [onAreaChange]{@link CommonMethod#onAreaChange(event: (oldValue: Area, newValue: Area) => void)}事件。子组件设置
 * > [position]{@link CommonMethod#position}属性时会固定子组件相对于Refresh组件的位置，导致子组件不会跟手进行下拉位移。
 * >
...
interface RefreshOptions {
   ...
   * [pullDownRatio]{@link CommonMethod#pullDownRatio}替代。
   *
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @FaAndStageModel
   * @crossplatform [since 10]
   ...
   * <br/>promptText设置有效时，
   * [refreshOffset]{@link CommonMethod#refreshOffset}属性默认值为96vp。
   * <br/>自定义文本最大的字体缩放倍数
   * [maxFontScale]{@link maxFontScale}为2。
   *
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
 ...
 * > [Web]{@link ./web}的联动。当[Swiper]{@link ./swiper}设置
...
declare class RefreshAttribute extends CommonMethod<RefreshAttribute> {
  ...
  /**
   * 进入刷新状态时触发回调，等同于onStateChange回调中state为Refresh的情况。若仅需监听刷新启动，使用onRefreshing更简洁；若需跟踪全部刷新状态变化（Inactive、Drag、OverDrag、
   * Refresh、Done），请使用onStateChange。
   *
   ...
   *
   * @param { number } value - 下拉偏移量，单位vp。<br/>取值范围：(0, +∞)。<br/>默认值：未设置[promptText]{@link RefreshOptions}参数时为64vp，设置了
   *     [promptText]{@link RefreshOptions}参数时为96vp。 <br/>如果取值为0或负数的时候此接口采用默认值。
   * @returns { RefreshAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   ...
   *
   * 未通过该接口设置时，当未设置[promptText]{@link RefreshOptions}参数时，默认偏移量为64vp；设置了[promptText]{@link RefreshOptions}参数时，默认偏移量为96vp。
   *
   * @param { number | Resource } value - 下拉偏移量。<br/>单位：vp<br/>取值范围：(0, +∞)。<br/>默认值：未设置
   *     [promptText]{@link RefreshOptions}参数时为64vp，设置了[promptText]{@link RefreshOptions}参数时为96vp。<br/>值为0或负数时，按默认值处理。
   * @returns { RefreshAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   ...
   * 设置上滑是否取消刷新。
   *
   * @param { boolean | undefined } enabled - 设置上滑是否取消刷新。<br/>true表示取消刷新；false表示不取消刷新。<br/>默认值：true。值为undefined时，按默认值处理。
   * @returns { RefreshAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   ...
   *     下拉跟手系数，下拉距离越大，跟手系数越小。<br/>取值范围：[0, 1]。小于0的值会被视为0，大于1的值会被视为1。
   ...
   * 设置最大下拉距离。
   *
   * @param { Optional<number> } distance - 最大下拉距离。<br/>取值范围：
   *     [0, +∞)，值小于0时按0处理。当该值小于刷新的下拉偏移量refreshOffset时，Refresh下拉离手不会触发刷新。<br/>undefined和null按没有设置此属性处理。<br/>默认值：undefined<br/>单位：vp
   * @returns { RefreshAttribute } Refresh组件的属性。
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
