...
interface PathInterface {
   ...
   *
   * @param { object } value - [since 7 - 17]
   * @param { PathOptions } [options] - Path组件绘制属性的配置对象。
   *     <br>省略时不设置绘制属性，组件按默认尺寸显示。默认尺寸根据路径内容自动计算宽度和高度。
 ...
 * <br>
 * y：直线终点的y轴坐标。                                    | 从当前点到给定的(x, y)坐标画一条线，该坐标成为新的当前点。例如，`L 50 50`表示绘制当前点到(50, 50)点的直线，并将
 * (50, 50)点作为新子路径的起始点。 |
 ...
 * x：终点参数的x坐标值。
 * <br>
 * y：终点参数的y坐标值。                        | 使用(x1, y1)作为曲线起点的控制点，(x2, y2)作为曲线终点的控制点，从当前点到(x, y)绘制三次贝塞尔曲线。例如，
 * `C100 100 250 100 250 200 `表示绘制当前点到(250, 200)点的三次贝塞尔曲线，并将(250, 200)点作为新子路径的起始点。 |
 ...
 * x：终点参数的x坐标值。
 * <br>
 * y：终点参数的y坐标值。                              |使用(x2, y2)作为曲线终点的控制点，从当前点到(x, y)绘制三次贝塞尔曲线。若前一个命令是C或S，则起点控制点是上一个命令的终点控制点相对于
 * 当前点的映射。例如，`C100 100 250 100 250 200 S400 300 400 200`第二段贝塞尔曲线的起点控制点为(250, 300)。如果没有前一个命令或者前一个命令不是 C或S，则第一个控制点与当前点重合。 
 * |
 ...
 * x：终点参数的x坐标值。
 * <br>
 * y：终点参数的y坐标值。                              | 使用(x1, y1)作为控制点，从当前点到(x, y)绘制二次贝塞尔曲线。例如，`Q400 50 600 300 `表示绘制当前点到(600, 3
 * 00)点的二次贝塞尔曲线，并将(600, 300)点作为新子路径的起始点。 |
 * | T    | smooth quadratic Bezier curveto | x：终点参数的x坐标值。
 * <br>
 * y：终点参数的y坐标值。                                    | 从当前点到(x, y)绘制二次贝塞尔曲线。若前一个命令是Q或T，则控制点是上一个命令的终点控制点相对于当前点的映射。 例如，
 * `Q400 50 600 300 T1000 300`第二段贝塞尔曲线的控制点为(800, 550)。 如果没有前一个命令或者前一个命令不是Q或T，则第一个控制点与当前点重合。 |
 ...
 * x：终点参数的x坐标值。
 * <br>
 * y：终点参数的y坐标值。 | 从当前点到(x, y)绘制一条椭圆弧。椭圆的大小和方向由两个半径(rx, ry)和x-axis-rotation定义，指示整个椭圆相对于当前坐标系如何旋转（以度为单位）。 large-arc-flag 和
 * sweep-flag确定弧的绘制方式。 |
 * | Z    | closepath                        | none                                     | 通过将当前路径连接回当前子路径的初始点来关闭当前子路径。             |
 * 
 * 例如：commands('M0 20 L50 50 L50 100 Z')定义了一个三角形，起始于位置(0, 20)，接着绘制点(0, 20)到点(50, 50)的直线，再绘制从点(50, 50)到点(50, 100)的直线，最后绘制
 * 从点(50, 100)到(0, 20)的直线关闭路径，形成封闭三角形。
 *
 * @syscap SystemCapability.ArkUI.ArkUI.Full
 * @crossplatform [since 10]
 * @form [since 9]
