...
declare class RectAttribute extends CommonShapeMethod<RectAttribute> {
  /**
   * 设置圆角的宽度。仅设置radiusWidth时，圆角的宽度和高度相同。该属性与[radius]{@link RectAttribute#radius}属性效果类似，当与radius组合使用时，radius属性优先于本属性生效。支持
   * [attributeModifier]{@link CommonMethod#attributeModifier}动态设置属性方法。异常值undefined、null、NaN和Infinity按照默认值处理。
   *
   * @param { number | string } value - 圆角的宽度，取值范围≥0。
   *     <br>默认值：0
   *     <br>默认单位：vp。
   *     <br>异常值undefined、null、NaN和Infinity按照默认值处理。 [since 7 - 19]
   * @param { Length } value - 圆角的宽度，取值范围≥0。
   *     <br>默认值：0
   *     <br>默认单位：vp。
   *     <br>异常值undefined、null、NaN和Infinity按照默认值处理。 [since 20]
   * @returns { RectAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @FaAndStageModel
   * @crossplatform [since 10]
  ...
  /**
   * 设置圆角的高度。仅设置radiusHeight时，圆角的高度和宽度相同。该属性与[radius]{@link RectAttribute#radius}属性效果类似，当与radius组合使用时，radius属性优先于本属性生效。支
   * 持[attributeModifier]{@link CommonMethod#attributeModifier}动态设置属性方法。异常值undefined、null、NaN和Infinity按照默认值处理。
   *
   * @param { number | string } value - 圆角的高度，取值范围≥0。
   *     <br>默认值：0
   *     <br>默认单位：vp。
   *     <br>异常值undefined、null、NaN和Infinity按照默认值处理。 [since 7 - 19]
   * @param { Length } value - 圆角的高度，取值范围≥0。
   *     <br>默认值：0
   *     <br>默认单位：vp。
   *     <br>异常值undefined、null、NaN和Infinity按照默认值处理。 [since 20]
   * @returns { RectAttribute }
  ...
  /**
   * 设置圆角半径大小，取值范围≥0，支持[attributeModifier]{@link CommonMethod#attributeModifier}动态设置属性方法。该属性与
   * [radiusWidth]{@link RectAttribute#radiusWidth}、[radiusHeight]{@link RectAttribute#radiusHeight}属性效果类似，在组合使用时优先于
   * radiusWidth和radiusHeight生效。异常值undefined、null、NaN和Infinity按照默认值处理。
   *
   * @param { number | string | Array<any> } value - 圆角半径大小。
   *     <br>默认值：0
   *     <br>默认单位：vp 
   *     <br>异常值undefined、null、NaN和Infinity按照[[0, 0], [0, 0], [0, 0], [0, 0]]处理。 [since 7 - 19]
   * @param { Length | Array<any> } value - 圆角半径大小。
   *     <br>默认值：0
   *     <br>默认单位：vp 
   *     <br>异常值undefined、null、NaN和Infinity按照[[0, 0], [0, 0], [0, 0], [0, 0]]处理。 [since 20]
   * @returns { RectAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @FaAndStageModel
   * @crossplatform [since 10]
