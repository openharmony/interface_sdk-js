...
interface PolylineInterface {
   ...
   * @param { PolylineOptions } [options] - Polyline绘制区域，用于设置Polyline组件的宽度和高度。当需要指定Polyline的绘制区域大小时传入此参数，不传入时使用默认宽度和高度（均
   *     为0）。
...
declare class PolylineAttribute extends CommonShapeMethod<PolylineAttribute> {
  /**
   * 设置折线经过坐标点列表，支持通过[attributeModifier]{@link CommonMethod#attributeModifier}动态设置属性。
   *
   * @param { Array<any> } value - 折线经过坐标点列表。使用时传入一个二维数组，每个子数组表示一个顶点的[x, y]坐标。
   *     <br>默认值：[]（空数组）
   *     <br>默认单位：vp 
