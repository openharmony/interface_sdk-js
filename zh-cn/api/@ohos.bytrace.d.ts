 ...
 * > **说明：**
 * >
 * > - 本模块接口从API version 8开始废弃，建议使用新接口[@ohos.hiTraceMeter]{@link @ohos.hiTraceMeter:hiTraceMeter}
 *
...
declare namespace bytrace {
   ...
   * > **说明：**
   * >
   * > - 如果有多个相同name的任务需要追踪或者对同一个任务要追踪多次，并且这些跟踪任务会同时被执行，则每次调用startTrace的taskId必须不一致。如
   * 果具有相同name的跟踪任务是串行执行的，则taskId可以相同。在下面bytrace.finishTrace的示例中会举例说明。
