...
declare namespace telephonyManager {
  ...
  /**
   * 禁用指定卡槽的SIM卡。禁用后，无法使用该卡槽的SIM卡接打电话、收发短信、上网。例如，企业设备管理员可在员工离职或设备丢失时，禁用SIM卡防止未授权使用。适用于企业需要限制员工设备通话能力的场景，例如员工离职或设备遗失时防止
   * SIM卡被滥用，保障企业通信安全和成本控制。
   *
   ...
   *
   * 已经通过[setDisallowedPolicy]{@link @ohos.enterprise.restrictions:restrictions.setDisallowedPolicy}接口禁用了设备通话能力，再通过本接口移除
   * 通话呼出的禁用或允许名单，返回203错误码。通过[setDisallowedPolicy]{@link @ohos.enterprise.restrictions:restrictions.setDisallowedPolicy}
   * 接口解除禁用设备通话能力后，可解除冲突。
   *
   * @permission ohos.permission.ENTERPRISE_MANAGE_TELEPHONY
   * @param { Want } admin - 企业设备管理扩展组件。Want中必须包含企业设备管理扩展能力的abilityName和所在应用的bundleName。
   ...
   *
   * 1. 已经通过[setDisallowedPolicy]{@link @ohos.enterprise.restrictions:restrictions.setDisallowedPolicy}接口禁用了设备通话能力，再通过本接口移除通话呼入的禁用或允许名单，返回203错误码。通过[setDisallowedPolicy]{@link @ohos.enterprise.restrictions:restrictions.setDisallowedPolicy}接口解除禁用设备通话能力后，可解除冲突。
   *
   * @permission ohos.permission.ENTERPRISE_MANAGE_TELEPHONY
   * @param { Want } admin - 企业设备管理扩展组件。Want中必须包含企业设备管理扩展能力的abilityName和所在应用的bundleName。
   * @param { adminManager.Policy } policy - 允许或禁用名单策略。BLOCK_LIST为禁用名单，TRUST_LIST为允许名单。
  ...
  /**
   * 获取设备当前默认使用的数据流量卡卡槽ID。例如，企业设备管理员可在设备管理过程中查询当前配置的默认数据流量卡，以便进行流量管理或切换数据卡配置。未插卡或者飞行模式下会获取上一次使用的数据流量卡卡槽ID、设备从未设置过默认数据流量卡
   * 场景下，该接口返回默认卡槽1，值为0。
   *
   * @permission ohos.permission.ENTERPRISE_MANAGE_TELEPHONY
   * @param { Want } admin - 企业设备管理扩展组件。Want中必须包含企业设备管理扩展能力的abilityName和所在应用的bundleName。
