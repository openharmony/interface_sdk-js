 ...
 * >
 * > - 在[BuilderNode]{@link BuilderNode}的情况下，目前在作为根节点的自定义组件中支持纹理导出的有以下组件：[Badge]{@link ../@internal/component/ets/badge}
 * > 、[Blank]{@link ../@internal/component/ets/blank}、[Button]{@link ../@internal/component/ets/button}、
 * > [CanvasGradient]{@link ../@internal/component/ets/canvas}、[CanvasPattern]{@link ../@internal/component/ets/canvas}、
 ...
 * > [Line]{@link ../@internal/component/ets/line}、[LoadingProgress]{@link ../@internal/component/ets/loading_progress}、
 ...
 * > [Video]{@link ../@internal/component/ets/video}（不含全屏播放能力）、[Web]{@link ../@internal/component/ets/web}、
 ...
 *
 * @unionmember { TouchEvent } 触摸事件。
 * @unionmember { MouseEvent } 鼠标事件。
 * @unionmember { AxisEvent } 轴事件。
 * @syscap SystemCapability.ArkUI.ArkUI.Full
 * @stagemodelonly
 * @crossplatform
 * @atomicservice
...
/**
 * 提供能够挂载系统组件的自定义节点BuilderNode。BuilderNode仅可作为叶子节点使用，支持通过@Builder生成组件树、实现组件复用与回收、跨节点事件分发以及状态同步，适用于在应用内动态创建和管理自定义组件节点的场景。
 * 使用方式参考[BuilderNode开发指南](docroot://ui/arkts-user-defined-arktsNode-builderNode.md)。
 * 
 ...
 * > [SymbolSpan]{@link ../@internal/component/ets/symbol_span}或自定义组件，将额外生成一个[FrameNode]{@link ./FrameNode}，在节点树中显示为“
 * > BuilderProxyNode”，这会导致树结构变化，影响事件传递等测试流程。详情参见
 * > [BuilderNode内的BuilderProxyNode导致树结构发生变化](docroot://ui/arkts-user-defined-arktsNode-builderNode.md#buildernode内的builderproxynode导致树结构发生变化)。
 * >
 * > - 如果在跨页面复用BuilderNode时显示异常，可参考[跨页面复用注意事项](docroot://ui/arkts-user-defined-arktsNode-builderNode.md#跨页面复用注意事项)。
 * >
 * > - 当前不支持在预览器中使用BuilderNode。
 * >
 * > - BuilderNode下的自定义组件支持使用[@Prop装饰器](docroot://ui/state-management/arkts-prop.md)。不支持使用
 * > [@Link装饰器](docroot://ui/state-management/arkts-link.md)来跨越BuilderNode同步外界的数据和状态。
 * >
 * > - 如果BuilderNode的子节点是自定义组件，不支持该自定义组件使用[@Reusable装饰器](docroot://ui/state-management/arkts-reusable.md)，详细内容参见
 * > [BuilderNode在子自定义组件中使用@Reusable装饰器](docroot://ui/arkts-user-defined-arktsNode-builderNode.md#buildernode在子自定义组件中使用reusable装饰器)。
 * >
 * > - 从API version 12开始，自定义组件支持接收[LocalStorage](docroot://ui/state-management/arkts-localstorage.md)实例。可以通过
 * > [传递LocalStorage实例](docroot://ui/state-management/arkts-localstorage.md#自定义组件接收localstorage实例)来使用LocalStorage相关的装饰器
 ...
 * >
 * > - 从API version 20开始，通过配置[BuildOptions]{@link BuildOptions}，内部自定义组件的
 * > [@Consume](docroot://ui/state-management/arkts-provide-and-consume.md)支持接收所在页面的
 * > [@Provide](docroot://ui/state-management/arkts-provide-and-consume.md)数据。
 * >
 * > - 其余装饰器行为未定义，不建议使用。
 * >
...
export class BuilderNode<Args extends Object[]> {
   ...
   *     [UIContext获取方法](docroot://reference/apis-arkui/js-apis-arkui-node.md#uicontext获取方法)。uiContext需要为一个有效的值，即UI上下文正
   *     确，如果传入非法值或者未设置，会导致创建失败。
   * @param { RenderOptions } options - BuilderNode的构造可选参数，参数用于构造节点的理想大小和节点的渲染类型。
   *     <br>默认值：undefined
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   ...
   * 接口，本接口支持builder的配置参数，用于配置Builder的构建行为，具体属性见[BuildOptions]{@link BuildOptions}。
   * 
   * > **说明：**
   * >
   ...
   *     [@Builder](docroot://ui/state-management/arkts-builder.md)。
   * @param { Object } arg - builder的入参。当前仅支持一个入参，且入参对象类型与@Builder定义的入参类型保持一致。
   * @param { BuildOptions } options - builder的配置参数，用于配置Builder的构建行为，具体属性和说明见[BuildOptions]{@link BuildOptions}。
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   * @atomicservice
  ...
  /**
   * 根据提供的参数更新BuilderNode，该参数与[build]{@link BuilderNode#build(builder: WrappedBuilder<Args>, arg?: Object)}方法调用时传入的参数类型相
   * 同。对自定义组件进行update的时候需要在自定义组件中将使用的变量定义为[@Prop](docroot://ui/state-management/arkts-prop.md)类型。
   *
   * @param { Object } arg - 用于更新BuilderNode的参数，和
   *     [build]{@link BuilderNode#build(builder: WrappedBuilder<Args>, arg?: Object)}调用时传入的参数类型一致。
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   * @atomicservice [since 12]
   ...
   *
   * @returns { FrameNode | null } BuilderNode持有的FrameNode对象，用于将该BuilderNode作为子节点挂载到其他FrameNode上。若该BuilderNode不包含
   *     FrameNode，则返回空对象null。
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   * @atomicservice [since 12]
   ...
   *
   * @param { TouchEvent } event - 用于派发到BuilderNode创建出的FrameNode上的触摸事件。
   * @returns { boolean } 派发事件是否成功。true：已命中响应事件的组件；false：未命中任何可响应事件的组件。
   *     <br>**说明：**
   *     <br>如果未按照预期命中组件，需要确认以下几点：
   *     <br>1. 坐标系是否转换正确。
   *     <br>2. 组件是否处于可交互状态。
   *     <br>3. 是否绑定事件。
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   ...
   * 
   * BuilderNode通过reuse和[recycle]{@link BuilderNode#recycle}完成其内外自定义组件之间的复用事件传递，具体使用场景请参见
   ...
   * >
   * > - 如果是开发者构造的事件，必填字段必须赋值，比如触摸事件的touches字段，轴事件的scrollStep字段。要保证事件的完整，比如触摸事件的[TouchType]{@link TouchType}中DOWN和UP字段都要
   * > 有，防止出现未定义行为。
   * >
   ...
   * > [TouchType]{@link TouchType}中必须同时包含DOWN和UP两个字段，防止出现程序异常或意外崩溃。
   * >
   * > - [webview]{@link @ohos.web.webview:webview}已经处理过坐标系变换，可以将事件直接下发。
   * >
   ...
   *
   * @param { InputEventType } event - 用于事件分发的输入事件。
   * @param { CompetitionStrategy } [competitionStrategy] - 分发事件的手势竞争策略。CompetitionStrategy.DEFAULT表示非竞争模式（目标组件与当前组件的手势不
   *     竞争），适用于当前组件与目标组件各自独立处理手势、无需竞争同一事件的场景；CompetitionStrategy.COMPETITION表示竞争模式（目标组件与当前组件的手势参与竞争），适用于当前组件与目标组件需要竞争同一
   *     手势事件的场景。不传入时默认为CompetitionStrategy.DEFAULT（非竞争）。
   ...
   *
   * @param { boolean } enabled - BuilderNode对象是否设置为继承父组件中自定义组件的冻结策略。
   *     <br>true：继承父组件中自定义组件的冻结策略；false：不继承父组件中自定义组件的冻结策略。
   *     <br>**说明：** 仅当父组件为自定义组件、BuilderNode、ComponentContent、ReactiveBuilderNode或ReactiveComponentContent时，设置为true才会生效。
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   * @atomicservice
...
/**
 * ReactiveBuilderNode支持通过无状态的UI方法[@Builder](docroot://ui/state-management/arkts-builder.md)生成组件树，并持有该组件树的根节点，不支持定义为状态变
 * 量。ReactiveBuilderNode中持有的[FrameNode]{@link ./FrameNode}仅用于将此ReactiveBuilderNode作为子节点挂载到其他FrameNode上。对
 * ReactiveBuilderNode持有的FrameNode进行属性设置与子节点操作可能会导致未定义行为，因此不建议通过ReactiveBuilderNode的
 * [getFrameNode]{@link BuilderNode#getFrameNode}方法和[FrameNode]{@link ./FrameNode}节点的
 * [getRenderNode]{@link ./FrameNode:FrameNode#getRenderNode}方法获取RenderNode，并通过
...
export class ReactiveBuilderNode<Args extends Object[]> {
   ...
   * [RenderOptions]{@link RenderOptions}中的[selfIdealSize]{@link RenderOptions}，否则ReactiveBuilderNode内的节点默认父组件布局约束为
   * [0, 0]。调用此接口，若不设置selfIdealSize则认为ReactiveBuilderNode中子树的根节点大小为[0, 0]。
   *
   * @param { UIContext } uiContext - UI上下文，获取方式可参考
   *     [UIContext获取方法](docroot://reference/apis-arkui/js-apis-arkui-node.md#uicontext获取方法)。uiContext需要为一个有效的值，即UI上下文正
   *     确，如果传入非法值或者未设置，会导致创建失败。
   * @param { RenderOptions } [options] - ReactiveBuilderNode的构造可选参数，参数用于构造节点的理想大小和节点的渲染类型。
   *     <br>默认值：undefined
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   ...
   * > - @Builder嵌套使用的时候需要保证内外的@Builder方法的入参对象一致。
   * >
   * > - 需要操作ReactiveBuilderNode中的对象时，需要保证其引用不被回收。当ReactiveBuilderNode对象被虚拟机回收之后，它的[FrameNode]{@link ./FrameNode}、
   * > [RenderNode]{@link ./RenderNode:RenderNode}对象也会与后端节点解引用。即从ReactiveBuilderNode中获取的FrameNode对象不对应任何一个节点。
   * >
   * > - ReactiveBuilderNode对象会持有实体节点的引用。如果不需要使用ReactiveBuilderNode前端对象管理后端节点，可以调用
   * > [dispose]{@link ReactiveBuilderNode#dispose}接口，实现前后端对象的解绑。
   *
   * @param { WrappedBuilder<Args> } builder - 创建对应节点树时所需的无状态UI方法
   *     [@Builder](docroot://ui/state-management/arkts-builder.md)。
   * @param { BuildOptions } config - 用于配置Builder的构建行为，BuildOptions中所有属性都是可选的，各属性默认值请参见BuildOptions的说明。
   * @param { Args } args - builder的入参，用于构造WrappedBuilder对象封装的builder函数。支持多个入参。默认值为undefined。
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   * @atomicservice
   ...
   *
   * @returns { FrameNode | null } ReactiveBuilderNode持有的FrameNode对象，用于将该ReactiveBuilderNode作为子节点挂载到其他FrameNode上。若该
   *     ReactiveBuilderNode不包含FrameNode，则返回空对象null。
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   * @atomicservice
  ...
  /**
   * 将原始事件派发到某个ReactiveBuilderNode创建的FrameNode上。适用于在自定义NodeContainer中将父组件接收的触摸事件转发给ReactiveBuilderNode内部组件，使内部组件能够响应触摸交互
   * 的场景。
   * 
   * postTouchEvent是从组件树的中间节点往下分发，需要变换到父组件坐标系才能分发成功，参考下图。
   * 
   ...
   * > 同一时间戳，postTouchEvent只能调用一次。
   *
   * @param { TouchEvent } event - 用于派发到ReactiveBuilderNode创建出的FrameNode上的触摸事件。
   * @returns { boolean } 派发事件是否成功。true：已命中响应事件的组件；false：未命中任何可响应事件的组件。
   *     <br>**说明：**
   *     <br>如果未按照预期命中组件，需要确认：
   *     <br>1. 坐标系是否转换正确。
  ...
  /**
   * 立即释放当前ReactiveBuilderNode对象对[实体节点](docroot://ui/arkts-user-defined-node.md#基本概念)的引用关系。关于ReactiveBuilderNode的解绑场景请参见
   * [节点解绑](docroot://ui/arkts-user-defined-arktsNode-builderNode.md#解除实体节点引用关系)。
   * 
   * > **说明：**
   * >
   ...
   * ReactiveBuilderNode的解绑场景请参见[节点解绑](docroot://ui/arkts-user-defined-arktsNode-builderNode.md#解除实体节点引用关系)。从API版本26.0.0
   * 开始，ReactiveBuilderNode中的自定义组件支持V2组件复用，请参见
   * [@ReusableV2装饰器：V2组件复用](docroot://ui/state-management/arkts-new-reusableV2.md)。
   * 
   * ReactiveBuilderNode通过reuse和[recycle]{@link ReactiveBuilderNode#recycle}完成其内外自定义组件之间的复用事件传递，具体使用场景请参见
   * [BuilderNode调用reuse和recycle接口实现节点复用能力](docroot://ui/arkts-user-defined-arktsNode-builderNode.md#buildernode调用reuse和recycle接口实现节点复用能力)。
   *
   ...
   * [@Reusable装饰器：V1组件复用](docroot://ui/state-management/arkts-reusable.md)。从API版本26.0.0开始，ReactiveBuilderNode中的自定义组件支持V
   * 2组件复用，请参见[@ReusableV2装饰器：V2组件复用](docroot://ui/state-management/arkts-new-reusableV2.md)。
   * 
   * ReactiveBuilderNode通过[reuse]{@link ReactiveBuilderNode#reuse}和recycle完成其内外自定义组件之间的复用事件传递，具体使用场景请参见
   * [BuilderNode调用reuse和recycle接口实现节点复用能力](docroot://ui/arkts-user-defined-arktsNode-builderNode.md#buildernode调用reuse和recycle接口实现节点复用能力)。
   *
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   ...
   * 时，需要在此类数据变更后手动调用此方法以更新数据，当使用V2装饰器（如@ObservedV2）装饰的类实例时，支持自动更新，无需手动调用。
   *
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
  ...
  /**
   * 将输入事件分发到ReactiveBuilderNode管理的目标节点。适用于在自定义NodeContainer中将父组件接收的触摸、鼠标或轴事件转发给ReactiveBuilderNode内部组件，使内部组件能够响应相应交互的场
   * 景。
   * 
   ...
   * > 转发的事件会在被分发到的目标组件所在的子树里做触摸测试（TouchTest），并触发对应手势，原始事件也会触发当前组件所在组件树中的手势。不保证两类手势的竞争结果。
   * >
   * > 如果是开发者构造的事件，必填字段必须赋值，比如触摸事件的touches字段、轴事件的scrollStep字段，同时要保证事件的完整，比如触摸事件的[TouchType]{@link TouchType}中DOWN和UP字段都要
   * > 有，防止出现未定义行为。
   * >
   * > [webview]{@link @ohos.web.webview:webview}已经处理过坐标系变换，可以将事件直接下发。
   * >
   ...
   *
   * @param { boolean } enabled - ReactiveBuilderNode对象是否设置为继承父组件中自定义组件的冻结策略。true为继承父组件中自定义组件的冻结策略，false为不继承父组件中自定义组件的冻结
   *     策略。仅当父组件为自定义组件、BuilderNode、ComponentContent、ReactiveBuilderNode或ReactiveComponentContent时，设置为true才会生效。
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
   * @atomicservice
  ...
  /**
   * 查询当前ReactiveBuilderNode对象是否已解除与后端实体节点的引用关系。前端节点均绑定有相应的后端实体节点，当节点调用dispose接口解除绑定后，再次调用接口可能会出现crash、返回默认值的情况。由于业务需求，可
   * 能存在节点在dispose后仍被调用接口的情况。为此，提供此接口以供开发者在操作节点前检查其有效性，避免潜在风险。
   *
   * @returns { boolean } 后端实体节点是否解除引用。true为节点已与后端实体节点解除引用，false为节点未与后端实体节点解除引用。
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
