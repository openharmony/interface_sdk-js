...
declare namespace uniformTypeDescriptor {
   ...
   * @param { Array<TypeDescriptor> } typeDescriptors - 注册的标准化数据类型描述符列表。TypeDescriptor用于描述自定义数据类型的属性，包括类型标识、所属类型、文件扩展名、MIME类型等。列表不可为空，
   * 其中元素个数不超过50。单个应用注册的标准化数据类型描述符数量总计不超过200。<br>
   * **TypeDescriptor格式要求：**<br>
   * 1.typeId不可为空字符串，长度不超过127，且仅包含字母、数字、中划线（-）和点号（.）。<br>
   * 2.belongingToTypes中元素个数不超过50。每项不可为空字符串，长度不超过127。<br>
   * 3.description/referenceURL/iconFile每项长度不超过255。<br>4.filenameExtensions中元素个数不超过50。每项不可为空字符串，长度不超过127且首字符必须为点号。<br>
   * 5.mimeTypes中元素个数不超过50。每项不可为空字符串，长度不超过127。<br>
   * **TypeDescriptor内容要求：**<br>
   * 1.typeId必须唯一，不能与已注册类型的typeId重复。<br>2.typeId必须以当前应用的包名开头。<br>3.belongingToTypes中的标准化数据类型ID必须为预置数据类型或本次注册的其他标准化数据类型的typeId。<br>
   ...
   * - 注销后，该类型将无法再被系统识别和使用。
   * @permission ohos.permission.MANAGE_DYNAMIC_UTD_TYPE
   * @param { Array<string> } typeIds - 待注销的typeId列表。列表不可为空，其中元素个数不超过50。每项长度不超过127。<br>
   * **typeId约束限制：**<br>1.typeId对应的标准化数据类型必须在系统中已注册；<br>
   * 2.typeId必须以当前应用的包名开头；<br>
