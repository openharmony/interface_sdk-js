...
declare namespace wantConstant {
  ...
  export enum Params {
     ...
     * 表示是否支持跨任务栈返回。
     *
     * 该参数用于控制跨应用的UIAbility返回逻辑，其核心作用是改变用户执行返回键时的应用跳转行为。例如现有UIAbility A和UIAbility B，
     * 当前前台显示的是UIAbility A，随后系统服务又拉起UIAbility B（同时在Want的Params字段配置该参数为true），那么，当UIAbility B
     * 退出时，会返回到UIAbility A（即返回到最近一次的访问任务）。如果未配置该参数，则默认直接返回桌面。需要注意的是，该字段仅支持系
     * 统设置，三方应用传入该字段不生效。
     *
     * @syscap SystemCapability.Ability.AbilityBase
     * @atomicservice [since 11]
