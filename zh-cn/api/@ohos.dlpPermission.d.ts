...
declare namespace dlpPermission {
    ...
    export interface DLPPermissionInfo {
        ...
        /**
         * 表示DLP文件的详细操作权限，取值范围由不同[ActionFlagType]{@link dlpPermission.ActionFlagType}的组合决定。
         *
         * @syscap SystemCapability.Security.DataLossPrevention
         * @since 10
         */
     ...
     *
     * @param { number } fd - 待查询文件的fd（文件描述符）。取值范围为[0, 2<sup>31</sup>-1]。当fd小于0时，抛出错误码19100001；当fd大于2<sup>31</sup>-1时，fd
     *     的值被截断。
     ...
     * @param { number } userId - 当前的用户ID，通过账号子系统获取的系统账号ID，默认主用户ID：100。<br>取值范围为[0, 2<sup>31</sup>-1]，超出范围将被截断。当传入参数值小于0
     *     时，输出错误日志，函数停止运行。
     * @param { number } appIndex - DLP沙箱号，即installDLPSandbox接口调用成功后的返回值，用于标识已安装的DLP沙箱。取值范围为[1000, 1100]。当小于0时，输出错误日志。
     * @returns { Promise<void> } Promise对象。无返回结果的Promise对象。
     * @throws { BusinessError } 201 - Permission denied.
     * @throws { BusinessError } 202 - Non-system applications use system APIs.
     * @throws { BusinessError } 401 - Parameter error. Possible causes: 1. Mandatory parameters are left unspecified.
    ...
    export interface DLPFile {
         ...
         * [deleteDLPLinkFile]{@link dlpPermission.DLPFile.deleteDLPLinkFile(linkFileName: string)}移除DLP link文件。
         * 
         ...
         * 
         * 调用stopFuseLink暂停FUSE关联读写后，必须调用[resumeFuseLink]{@link dlpPermission.DLPFile.resumeFuseLink()}恢复读写功能。
         * 
         ...
         * 
         * 必须在调用[stopFuseLink]{@link dlpPermission.DLPFile.stopFuseLink()}暂停读写后才能调用此方法恢复读写功能。
         * 
         ...
         * 
         * 在调用deleteDLPLinkFile前需要调用[addDLPLinkFile]{@link dlpPermission.DLPFile.addDLPLinkFile(linkFileName: string)}添加
         * DLP link文件。
         * 
         ...
         * 
         * 调用[openDLPFile]{@link dlpPermission.openDLPFile(ciphertextFd: number, appId: string)}成功后返回DLPFile对象，必须在使用完毕后调
         * 用closeDLPFile()释放资源。
         * 
     ...
     *
     * @permission ohos.permission.ACCESS_DLP_FILE
     * @param { number } plaintextFd - 待加密明文文件的fd。取值范围为[0, 2<sup>31</sup>-1]。当fd小于0时，打印错误日志，函数停止运行；当fd大于2<sup>31</sup>-1
     *     时，fd的值被截断。
     * @param { number } ciphertextFd - 目标加密文件的fd。取值范围为[0, 2<sup>31</sup>-1]。当fd小于0时，打印错误日志，函数停止运行；当fd大于2<sup>31</sup>-1
     *     时，fd的值被截断。
     ...
     *     时，fd的值被截断。
     * @param { DLPProperty } property - 授权用户信息：授权用户列表、owner账号、联系人账号。
     * @param { AsyncCallback<DLPFile> } callback - 回调函数。当生成DLP文件成功，err为undefined，data为获取到的DLP文件信息；否则为错误对象。
     * @throws { BusinessError } 201 - Permission denied.
     * @throws { BusinessError } 202 - Non-system applications use system APIs.
     * @throws { BusinessError } 401 - Parameter error. Possible causes: 1. Mandatory parameters are left unspecified.
     *     2. Incorrect parameter types.
     ...
     * 
     * 调用openDLPFile()成功后返回DLPFile对象，必须在使用完毕后调用[closeDLPFile]{@link dlpPermission.DLPFile.closeDLPFile()}释放资源。
     * 
