 ...
 * <br>
 * <br>谓词的接口函数与数据库的筛选条件一一对应，开发者在使用前需了解数据库相关知识。
 * <br>
 * <br>谓词(DataSharePredicates)的使用场景如下：
 * <br>
 * <br>- 用于在媒体文件管理服务作为检索条件使用，参考相册管理模块检索条件[FetchOptions]{@link @ohos.file.photoAccessHelper:photoAccessHelper.FetchOptions}。此
 * 种场景下，开发者无需关注数据库类型。
 * <br>
 * <!--Del-->
 * 
 * <br>- 用于调用[关系型数据库](docroot://reference/apis-arkdata/js-apis-data-relationalStore-sys.md)和
 * [分布式键值数据库](docroot://reference/apis-arkdata/js-apis-distributedKVStore-sys.md)系统接口时的检索条件使用。此种场景下，开发者根据使用的数据库类型参考对应的谓词
 * 使用方法。
 * 
 * <!--DelEnd-->
 *
...
declare namespace dataSharePredicates {
  ...
  class DataSharePredicates {
     ...
     * <br>
     * <br>目前仅关系型数据库及键值型数据库支持该谓词。
     *
     * @param { string } field - 数据库表中的列名。
     *     <br>field为undefined或null时，此次调用接口配置的谓词无效。
     * @param { ValueType } value - 指示要与谓词匹配的值。
     *     <br>value为undefined或null时，此次调用接口配置的谓词无效。
     ...
     *
     * @param { string } field - 数据库表中的列名。
     *     <br>当field为undefined或null时，此次调用接口配置的谓词无效。
     *     <br>当field为字符串'null'或'undefined'时，键值型数据库和关系型数据库接口使用该谓词时，可能匹配结果非预期或抛出异常。
     * @param { ValueType } value - 指示要与谓词匹配的值。
     *     <br>value为undefined或null时，此次调用接口配置的谓词无效。
     * @returns { DataSharePredicates } 返回与指定字段匹配的谓词。
     * @syscap SystemCapability.DistributedDataManager.DataShare.Core
     ...
     *     <br>field为undefined或null时，此次调用接口配置的谓词无效。
     *     <br>当field为字符串'null'或'undefined'时，键值型数据库和关系型数据库接口使用该谓词时，可能匹配结果非预期或抛出异常。
     * @param { string } value - 指示要与谓词匹配的通配符表达式。 
     *     <br>表达式中'%'代表零个、一个或多个数字或字符，'_'代表一个单一的数字或字符，不区分大小写。
     *     <br>value为undefined或null时，此次调用接口配置的谓词无效。
     * @returns { DataSharePredicates } 返回与指定字段匹配的谓词。
     * @syscap SystemCapability.DistributedDataManager.DataShare.Core
     * @systemapi [since 9 - 22]
     ...
     * @param { string } field - 数据库表中的列名。
     * @param { string } value - 指示要与谓词匹配的通配符表达式。
     *     <br>表达式中'*'代表零个、一个或多个数字或字符，'?'代表一个单一的数字或字符，区分大小写。
     * @returns { DataSharePredicates } 返回与指定字段匹配的谓词。
     * @syscap SystemCapability.DistributedDataManager.DataShare.Core
     * @systemapi
     * @StageModelOnly
     ...
     *     <br>low为number时，按数值排序指定匹配范围。
     *     <br>low为string时，按字典序排序指定匹配范围。
     *     <br>low为boolean时，按数值排序指定匹配范围。
     * @param { ValueType } high - 指示与谓词匹配的最大值。
     *     <br>high为number时，按数值排序指定匹配范围。
     *     <br>high为string时，按字典序排序指定匹配范围。
     *     <br>high为boolean时，按数值排序指定匹配范围。
     * @returns { DataSharePredicates } 返回与指定字段匹配的谓词。
     * @syscap SystemCapability.DistributedDataManager.DataShare.Core
     * @systemapi [since 9 - 22]
     * @publicapi [since 23]
     ...
     *
     * @param { string } field - 数据库表中的列名。
     *     <br>当field为null或undefined时，此次调用接口配置的谓词无效。
     *     <br>当field为字符串'null'或'undefined'时，键值型数据库和关系型数据库接口使用该谓词时，可能匹配结果非预期或抛出异常。
     * @param { ValueType } value - 指示要与谓词匹配的值。
     *     <br>value为undefined或null时，此次调用接口配置的谓词无效。
     * @returns { DataSharePredicates } 返回与指定字段匹配的谓词。
     ...
     * <br>目前仅关系型数据库及键值型数据库支持该谓词。
     *
     * @param { int } total - 最大数据记录数。
     *     <br>当使用键值型数据库且total为undefined或null时，会限制最大记录数为0。
     *     <br>当使用关系型数据库且total为undefined或null时，不会限制最大记录数。
     *     <br>当使用键值型数据库时，取值范围参考[键值型数据库limit接口]{@link @ohos.data.distributedKVStore:distributedKVStore.Query#limit}中的
     *     total参数说明。
     *     <br>当使用关系型数据库时，取值范围参考
     *     [关系型数据库limitAs接口]{@link @ohos.data.relationalStore:relationalStore.RdbPredicates#limitAs}中的value参数说明。
     * @param { int } offset - 指定查询结果的起始位置。
     *     <br>当offset为undefined或null时，起始位置为结果集的最前端。
     *     <br>当使用键值型数据库时，取值范围参考[键值型数据库limit接口]{@link @ohos.data.distributedKVStore:distributedKVStore.Query#limit}中的
     *     offset参数说明。
     *     <br>当使用关系型数据库时，取值范围参考
     *     [关系型数据库offsetAs接口]{@link @ohos.data.relationalStore:relationalStore.RdbPredicates#offsetAs}中的rowOffset参数说明。
     * @returns { DataSharePredicates } 返回与指定字段匹配的谓词。
     * @syscap SystemCapability.DistributedDataManager.DataShare.Core
     * @stagemodelonly
     * @crossplatform [since 12]
