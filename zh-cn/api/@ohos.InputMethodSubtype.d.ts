 ...
 * <br>
 * <br>本模块提供输入法子类型的属性描述能力。通过`InputMethodSubtype`可获取子类型的标识（`id`、`name`）、语言和区域（`locale`、`language`）、显示标签（`label`）、模式（`mode`：大写
 * /小写）、图标等属性，用于输入法子类型的识别、展示和切换。
 * <br>
 * <br>当需要查询、展示或切换输入法的不同语言/模式子类型时使用本模块。典型场景包括：系统设置应用展示输入法子类型列表供用户选择、输入法应用根据子类型信息切换语言或模式、应用获取当前输入法子类型信息等。
 * <br>
 * <br> > **说明：**
 * <br> >
 * <br> > - 本模块同时支持ArkTS-Dyn、ArkTS-Sta。
 * <br> > `InputMethodSubtype`是纯数据定义模块，其对象由系统框架创建和返回，开发者不可自行构造。典型跨模块使用流程：
 * <br>
 * <br>1. 编辑框应用侧（`@ohos.inputMethod`模块）：通过`InputMethodSetting.listInputMethodSubtype()`查询输入法子类型列表，通过`inputMethod.switchCurrentInputMethodSubtype()`切换到指定子类型。
 * <br>2. 输入法应用侧（`@ohos.inputMethodEngine`模块）：通过`InputMethodAbility.on('setSubtype')`监听子类型切换事件，回调参数为`InputMethodSubtype`对象，据此调整键盘布局和语言。
 * <br>
 * <br>本模块的核心开放能力由以下关键接口承载：
 * <br>
 * | Interface | 说明 |
 * |---|---|
 * | InputMethodSubtype | 输入法子类型属性接口，描述输入法的一种具体语言或模式形态。必填属性包括：`name`（所属应用包名）、`id`（子类型ID）、`locale`（区域）、`language`（语言）；
 * 可选属性包括：`label`（标签）、`labelId`（标签资源号）、`mode`（大写/小写模式）、`icon`（图标）、`iconId`（图标ID）、`extra`（额外信息）。 |
 * <br>
 * <br>本模块为纯数据定义模块，`InputMethodSubtype`作为子类型描述数据需与其他模块的API组合使用。典型组合场景为：在`@ohos.inputMethod`模块中，通过`InputMethodSetting`查询和切换子类
 * 型；在`@ohos.inputMethodEngine`模块中，通过`InputMethodAbility`监听子类型切换事件。
 * <br>
 * <br> > **说明：**
 * <br> >
 * <br> > `InputMethodSubtype`对象由系统框架创建和返回，开发者通过其他模块的查询接口获取，不可自行构造。
 *
