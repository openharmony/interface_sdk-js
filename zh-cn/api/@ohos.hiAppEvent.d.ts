 ...
 * > **说明：**
 * >
 * > - 本模块接口从API version 9开始废弃，建议使用新接口[@ohos.hiviewdfx.hiAppEvent]{@link @ohos.hiviewdfx.hiAppEvent:hiAppEvent}替代。
 *
...
declare namespace hiAppEvent {
   ...
   * @param { object } keyValues 事件参数。
   * @returns { Promise<void> } Promise对象，可以在其then()、catch()方法中分别对事件写入成功、写入异常的情况进行异步处理。
   * @syscap SystemCapability.HiviewDFX.HiAppEvent
   * @since 7 dynamiconly
   * @deprecated since 9
   ...
   * @param { EventType } eventType 事件类型。
   * @param { object } keyValues 事件参数。
   * @param { AsyncCallback<void> } [callback] 事件回调函数。
   * @syscap SystemCapability.HiviewDFX.HiAppEvent
   * @since 7 dynamiconly
   * @deprecated since 9
   * @useinstead ohos.hiviewdfx.hiAppEvent/hiAppEvent#write
   ...
   * @returns { boolean } 配置结果，true?表示配置成功，false?表示配置失败。
   * @syscap SystemCapability.HiviewDFX.HiAppEvent
   * @since 7 dynamiconly
   * @deprecated since 9
