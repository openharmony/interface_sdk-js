...
/**
 * fileAccess模块是基于[extension](docroot://application-models/extensionability-overview.md)机制实现的一个对公共文件访问和操作的
 * 框架。该模块一方面对接各类文件管理服务，如存储管理服务等；另一方面为系统应用提供一套统一的文件访问管理接口。存储管理服务可以管理内置存储部分目录，
 * 以及共享盘、U盘、SD卡等设备上的资源。
 *
 * > **说明：**
 ...
 * > - 推荐使用[@ohos.file.fs]{@link @ohos.file.fs:fileIo}接口进行文件访问。
 *
