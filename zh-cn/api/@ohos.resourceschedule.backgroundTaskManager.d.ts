...
declare namespace backgroundTaskManager {
   ...
   * APIs. Note that:
   * 
   * 1. When requesting a continuous task via [startBackgroundRunning()]{@link backgroundTaskManager.startBackgroundRunning(context: Context, request: ContinuousTaskRequest)}, notifications will be combined if the main type and subtype of the continuous task to be requested are the same as those of the existing continuous task in the current application, and the **combinedTaskNotification** value is **true** for both tasks. Otherwise, notifications will not be combined.
   * 2. Notifications will not be combined if the continuous task has no notification. For details about whether notifications are sent for the continuous task, see [BackgroundTaskMode]{@link backgroundTaskManager.BackgroundTaskMode}.
   ...
   * 6. When the [updateBackgroundRunning()]{@link backgroundTaskManager.updateBackgroundRunning(context: Context, request: ContinuousTaskRequest)} API is called to update a continuous task, the input **continuousTaskId** must exist. Otherwise, the update fails.
   * 7. Continuous tasks of the [MODE_SPECIAL_SCENARIO_PROCESSING]{@link backgroundTaskManager.BackgroundTaskMode} type are supported since API version 22. This task type must be used independently and notifications cannot be combined. Specifically, when you request or update a continuous task, it must be of the **MODE_SPECIAL_SCENARIO_PROCESSING** type. Otherwise, an error is returned.
   *
  ...
  export class ContinuousTaskRequest {
     ...
     *
     * @permission ohos.permission.KEEP_BACKGROUND_RUNNING
     * @param { Context } context - Application context.<br>For details about the application context of the FA model,
   ...
   *     [Context]{@link ./application/Context:Context}.<br> Note: Continuous tasks can be requested only by the
   *     UIAbility in the stage model and the ServiceAbility in the FA model.
   * @param { string[] } bgModes - Types of continuous tasks. For details about the available options, see
   ...
   *     [Context]{@link ./application/Context:Context}.<br> Note: Continuous tasks can be requested only by the
   *     UIAbility in the stage model and the ServiceAbility in the FA model.
   * @param { string[] } bgModes - Types of continuous tasks after the update. For details about the available options,
   ...
   * 
   * 1. This API can only update continuous tasks requested via [startBackgroundRunning(context: Context, request: ContinuousTaskRequest): Promise&lt;ContinuousTaskNotification&gt;]{@link backgroundTaskManager.startBackgroundRunning(context: Context, request: ContinuousTaskRequest)}.
   ...
   *     [Context]{@link ./application/Context:Context}.<br> Note: Continuous tasks can be requested only by the
   *     UIAbility in the stage model and the ServiceAbility in the FA model.
   * @param { int } continuousTaskId - Continuous task ID.<br>Note: You can obtain the ID of the current continuous task
  ...
  export enum BackgroundTaskMode {  
     ...
     * **NOTE**
     * 
     * 1. During data transfer, the application needs to update the progress. If the progress is not updated for more than 10 minutes, the continuous task of the **DATA_TRANSFER** type will be canceled.
     * 2. The notification type of the progress update must be live view. For details, see the example in [startBackgroundRunning()]{@link backgroundTaskManager.startBackgroundRunning(context: Context, bgModes: string[], wantAgent: WantAgent)}.
     *
     * @syscap SystemCapability.ResourceSchedule.BackgroundTaskManager.ContinuousTask
     * @since 21 dynamic
     * @since 24 static
     ...
     * **NOTE**
     * 
     * 1. If an application needs to run in the background for a long time, it can request user authorization through the [requestAuthFromUser]{@link backgroundTaskManager.ContinuousTaskRequest.requestAuthFromUser} API and check the authorization result via [checkSpecialScenarioAuth]{@link backgroundTaskManager.ContinuousTaskRequest.checkSpecialScenarioAuth}.
     * 2. Since API version 24, this capability is available only to applications that have obtained the ACL permission [ohos.permission.KEEP_BACKGROUND_RUNNING_SPECIAL_SCENARIO](docroot://security/AccessToken/restricted-permissions.md#ohospermissionkeep_background_running_special_scenario). For API version 23 and earlier, this capability is available only to applications that have obtained the ACL permission [ohos.permission.KEEP_BACKGROUND_RUNNING_SYSTEM](docroot://security/AccessToken/restricted-permissions.md#ohospermissionkeep_background_running_system). Applications that have obtained this permission are not affected for API version 24 and later.
     * 3. This task type must be used independently and notifications cannot be combined. Specifically, when you request or update a continuous task, it must be of the **MODE_SPECIAL_SCENARIO_PROCESSING** type. Otherwise, an error is returned.
     *
     * @syscap SystemCapability.ResourceSchedule.BackgroundTaskManager.ContinuousTask
     * @since 22 dynamic
     * @since 24 static
