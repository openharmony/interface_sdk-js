...
declare namespace pointer {
   ...
   * [setCursor]{@link @ohos.arkui.UIContext:CursorController#setCursor}，使用callback异步回调。
   *
  ...
  /**
   * 设置指定窗口的鼠标样式类型，此接口仅支持设置本应用进程内窗口的鼠标样式类型，如需通过UIExtensionAbility进程设置宿主窗口的鼠标样式类型，请参阅
   * [setCursor]{@link @ohos.arkui.UIContext:CursorController#setCursor}，使用Promise异步回调。
   *
   * @param { int } windowId - 窗口ID。取值范围为大于等于0的整数。<br>窗口ID合法并且对应窗口存在时，可以设置窗口的鼠标光标样式。<br>窗口ID合法但窗口不存在时，也可以设置鼠标光标样式。<br>设置
   ...
   * 鼠标光标样式。<br>如果通过[setPointerStyle]{@link pointer.setPointerStyle(windowId: int, pointerStyle: PointerStyle, callback: AsyncCallback<void>)}
   ...
   * [setCustomCursor]{@link @ohos.arkui.UIContext:CursorController#setCustomCursor}，使用
   * Promise异步回调。<br>应用窗口布局改变、热区切换、页面跳转、光标移出再回到窗口、光标在窗口不同区域移动，以上场景可能导致光标切换回系统样式，需要开发者重新设置光标样式。
   *
   ...
   * [setCustomCursor]{@link @ohos.arkui.UIContext:CursorController#setCustomCursor}。<br>应用窗口布局改变、热区切换、页面跳转、光标移出再回到窗口、光标在窗口不同区域移动，
   * 以上场景可能导致光标切换回系统样式，需要开发者重新设置光标样式。
   *
   * @param { int } windowId - 窗口ID。取值为大于0的整数。
   * @param { image.PixelMap } pixelMap - 自定义光标资源。
   * @param { int } [focusX] - 自定义光标焦点x，取值范围：大于等于0，默认为0，单位为像素（px）。
   ...
   * [光标资源图大小, 256×256]。
   * @returns { Promise<void> } Promise对象，无返回结果。
   * @throws { BusinessError } 401 - Parameter error. Possible causes: 1. Abnormal windowId parameter passed in. 2. Abnormal pixelMap parameter passed in; 3. Abnormal focusX parameter passed in.4. Abnormal focusY parameter passed in.
