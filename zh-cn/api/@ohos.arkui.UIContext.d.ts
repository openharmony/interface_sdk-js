...
export interface TargetInfo {
   ...
   * 
   * 2. 当id是string时，对应[通用属性id]{@link ./@internal/component/ets/common}所指定的组件，
   * 此id的唯一性需由开发者确保，但实际会有存在多个相同id的组件的可能性。
   *
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
  ...
  /**
   * 目标节点所在的自定义组件的UniqueID。当上述id指定为string类型且需要在指定自定义组件范围内查找目标节点时，
   * 可通过此属性圈定范围，方便开发者在一定范围内保证id: string的唯一性。默认不指定自定义组件范围。
   *
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @stagemodelonly
   * @crossplatform
