...
export default class EnterpriseAdminExtensionAbility {
   ...
   *
   * @param { common.StartupScene } scene - 开机向导完成场景，标识触发回调的具体场景类型，如用户设置完成（USER_SETUP）、OTA升级完成（OTA）、
   *     设备配置完成（DEVICE_PROVISION）等，开发者可根据不同场景执行相应的业务逻辑。
   * @syscap SystemCapability.Customization.EnterpriseDeviceManager
   * @stagemodelonly
   * @since 24
   */
   ...
   * > [systemManager.finishLogCollected]{@link @ohos.enterprise.systemManager:systemManager.finishLogCollected}删除已收集到的日志。
   *
   ...
   *
   * @param { systemManager.KeyEvent } keyEvent - 当前发生的按键事件信息，包含按键码（keyCode）、按键动作（keyAction，如按下/抬起）、触发时间（actionTime）、
   *     已按下按键列表（keyItems）等，用于识别和处理用户的按键操作。
   * @syscap SystemCapability.Customization.EnterpriseDeviceManager
   * @stagemodelonly
   * @since 23
   */
   ...
   *
   * @param { common.PolicyChangedEvent } event - 策略变更事件，包含策略变更的详细信息，如变更的应用包名（bundleName）、变更的函数名（functionName）、
   *     变更的参数（parameters）和变更时间（time），超级设备管理应用可根据这些信息进行策略审计或同步。
   * @syscap SystemCapability.Customization.EnterpriseDeviceManager
   * @stagemodelonly
   * @since 26.0.0
   */
