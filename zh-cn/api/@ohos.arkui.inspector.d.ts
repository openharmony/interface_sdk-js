...
declare namespace inspector {
  ...
  interface ComponentObserver {
    ...
    /**
     * 通过句柄为指定组件注册回调，当组件布局完成时会触发该回调。请注意，该接口无法监听窗口尺寸变化，相关需求请参考on('windowSizeChange')。此外，布局回调和窗口尺寸变化回调之间不存在确定的执行顺序依赖。
     *
     ...
     * @param { string } type - 必须填写字符串'layout'。<br>layout：组件布局完成。 [since 10 - 11]
     * @param { ()=>void } callback - 监听layout的回调。 [since 10 - 11]
     * @param { 'draw' } type - 必须填写字符串'layout'。<br>layout：组件布局完成。 [since 12]
     * @param { function } callback - 监听layout的回调。 [since 12]
     * @syscap SystemCapability.ArkUI.ArkUI.Full
     * @stagemodelonly
     * @crossplatform
     ...
     * 通过句柄为指定组件注册回调，当组件布局完成时会触发该回调。请注意，该接口无法监听窗口尺寸变化，相关需求请参考on('windowSizeChange')。此外，布局回调和窗口尺寸变化回调之间不存在确定的执行顺序依赖。
     *
     * @param { 'drawChildren' } type - 必须填写字符串'layout'。<br>layout：组件布局完成。
     * @param { Callback<void> } callback - 监听layout的回调。
     * @syscap SystemCapability.ArkUI.ArkUI.Full
     * @stagemodelonly
     * @crossplatform
     * @atomicservice
     ...
     *         callback需要和on('layout')方法中的callback为相同对象时才能取消回调成功。
     * @syscap SystemCapability.ArkUI.ArkUI.Full
     * @stagemodelonly
     * @crossplatform
     * @atomicservice
    ...
    /**
     * 通过ComponentObserver注册drawChildren事件回调。使用callback异步回调。与on('drawChildren')相比，
     * 本方法在回调中额外返回子组件的uniqueId信息（Callback<number[]>），便于开发者定位具体子组件。如需获取子组件标识，建议使用本方法；若不需要子组件信息，两者均可使用。
     * 以当前注册事件回调的节点为根节点，当组件的子组件位于UI组件主树中且绘制送显完成时，会触发该回调。如果组件树中存在多个drawChildren事件回调，只会触发最顶层的drawChildren事件回调。
     * 取消最顶层的回调后，其余drawChildren事件回调也无法生效。当前节点注册事件回调后，不支持修改其在UI组件主树中的层级位置。如需调整，请先取消事件回调，再重新注册事件回调。
     * 以下示例展示了inspector注册组件绘制送显完成回调通知能力的基本用法。通过onDrawChildren<sup>24+</sup>接口注册回调，当子树内节点完成渲染时，回调返回该节点的uniqueId信息。
     *
     ...
     * 要实现在子组件布局完成后停止触发特定回调，只需通过ComponentObserver句柄，取消注册该回调即可。如果组件树中存在多个layoutChildren事件回调，取消最顶层的回调后，
     * 其余layoutChildren事件回调也无法生效。
     * 以下示例展示了inspector注册组件布局和组件绘制送显完成回调通知能力的基本用法。同时，通过onLayoutChildren<sup>23+</sup>接口监听子树中的节点完成布局时的回调事件。
     *
     * @param { Callback<void> } [callback] - 需要取消注册的回调，如果参数缺省则取消注册该句柄下所有的回调。
