...
def compile_package(options):
    ...
    process = subprocess.Popen([npm, "install"], env=custom_env, cwd=tool_path, shell=False)
    process.wait()

    process1 = subprocess.Popen([npm, "run", "compile:arkui"], env=custom_env, cwd=tool_path, shell=False)
    process1.wait()
