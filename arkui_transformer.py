...
def compile_package(options):
    ...
    process = subprocess.run([npm, "install"], env=custom_env, cwd=tool_path, check=True, shell=False)

    process1 = subprocess.run([npm, "run", "compile:arkui"], env=custom_env, cwd=tool_path, check=True, shell=False)
